...
let tokenize input = 
	...
	let rec next_token str pos = 
		...
		(* Tok_Type_Int *)			
			...
			if (Str.string_match re_extra str Str.match_end())
				then let token_id = Str.matched_string str in
				(Tok_ID)::(next_token str Str.match_end())
			else (Tok_Type_Int (token_key^token_id)::(next_token str Str.match_end())
		
		(* ADD MORE TOKENS HERE *)
		
		else if (Str.string_match re_while str pos) 
			then let token = Str.matched_string str in
			(Tok_While)::(next_token str Str.match_end())
